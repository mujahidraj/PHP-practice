<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Appointment Form</title>
    <link rel="stylesheet" href="../design/design_agentappointment.css">
</head>
<body>
<form name="appointmentForm" method="post" action="" onsubmit="return validateForm()">
        <fieldset>
        <h1 align="center">Appointment Form</h1>
            <table cellpadding="5" align="center">
                <tr>
                    <td><h2>Agent Information</h2></td>
                    <td></td>
                </tr>
                <tr>
                    <td><b>Full Name :</b></td>
                    <td><input type="text" name="name" ></td>
                </tr>
                <tr>
                    <td><b>Email :</b></td>
                    <td><input type="email" name="email" ></td>
                </tr>
                <tr>
                    <td><b>Phone Number :</b></td>
                    <td><input type="text" name="phn_no" ></td>
                </tr>
                <hr>
                <tr>
                    <td><h2>Appointment Details</h2></td>
                    <td></td>
                </tr>
                <tr>
                    <td><b>Date :</b></td>
                    <td><input type="date" name="date" ></td>
                </tr>
                <tr>
                    <td><b>Time :</b></td>
                    <td><input type="time" name="time" ></td>
                </tr>
                <tr>
                    <td><b>Duration :</b></td>
                    <td><input type="text" name="duration" ></td>
                </tr>
                <tr>
                    <td><h2>Property Information</h2></td>
                    <td></td>
                </tr>
                <tr>
                    <td><b>Property Address :</b></td>
                    <td><input type="text" name="pro_add" ></td>
                </tr>
                <tr>
                    <td><b>MLS Number :</b></td>
                    <td><input type="text" name="mls_no" ></td>
                </tr>
                <tr>
                    <td><h2>Client Information</h2></td>
                    <td></td>
                </tr>
                <tr>
                    <td><b>Name :</b></td>
                    <td><input type="text" name="a_name" ></td>
                </tr>
                <tr>
                    <td><b>Email :</b></td>
                    <td><input type="text" name="a_email" ></td>
                </tr>
                <tr>
                    <td><b>Phone Number :</b></td>
                    <td><input type="text" name="a_phn" ></td>
                </tr>
                <tr>
                    <td colspan="2" align="center"><input type="submit" value="Submit" name="submit"></td>
                </tr>
            </table>
        </fieldset>
    </form>
    <script src="script/agent_appointment.js"></script>
</body>
</html>
